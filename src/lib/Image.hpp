...
namespace cvip
{
    bool compareRect(cv::Rect r1, cv::Rect r2);


    class Mat3 {
        std::shared_ptr<cv::Mat> mat;

        void initFromFrames(const std::vector<cv::Mat>& ims);
    public:

        //! initialise straight from images
        Mat3(const std::vector<cv::Mat> &ims, bool normalise = false);

        //! initialise database sample
        Mat3(const std::string& basepath, size_t T);

        //! initialise from pair
        Mat3(const cv::Mat& im1, const cv::Mat& im2);

        ~Mat3();

        //! synthesise a new matrix by repeating the tTH frame of the sequence T times
        Mat3 syntehsiseFromTthFrame(size_t t) const;

        std::vector<cv::Mat> frames;

        bool areFramesLegal() const;

        size_t T() const { return mat->size[0]; }
        size_t rows() const { return mat->size[1]; }
        size_t cols() const { return mat->size[2]; }

        std::vector<cv::Mat> split() const;
    };
    ...
    public:
        ...
        // default constructor
        Rect(cv::Rect& rect) : x1(rect.x), y1(rect.y), x2(rect.x+rect.width-1), y2(rect.y+rect.height-1), width(rect.width), height(rect.height) {}
    ...
    public:
        ...
        static cv::Mat doubleToUcharNoNorm(const cv::Mat& input);

        static std::vector<double> columnToVector(const cv::Mat& col);

        static cv::Mat vectorToColumn(const std::vector<double>& vec);

        static double matEnergy(const cv::Mat& mat);
        ...
        static std::vector<int> readFileToIntVector(const std::string& FileName);

        static void writeVectorToFile(const std::vector<double>& doubles, const std::string& FileName);

        static void writeVectorToFile(const std::vector<int>& ints, const std::string& FileName);

        static void writeValueToFile(double value, const std::string& fileName);
        ...
        // lbp transform of image
        static void mct(const cv::Mat &I, cv::Mat &mI);

        static void show(const cv::Mat& I, const std::string& winName = "asdasd");
        ...
        static cv::Mat readFromFile(const std::string& FileName, int cols);
        static double readValueFromFile(const std::string& FileName);
        ...
        static void zNormalize(cv::Mat& input, double mean, double std);
