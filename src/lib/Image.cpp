...
bool cvip::compareRect(cv::Rect r1, cv::Rect r2)
{
...
Mat3::Mat3(const std::vector<cv::Mat> &ims, bool normalise)
    : frames(ims)
{

    for (size_t i=0; i<frames.size(); ++i) {
        if (frames[i].type() != CV_64FC1)
            frames[i].convertTo(frames[i], CV_64FC1);

        if (normalise)
            frames[i] = frames[i]/255;
    }

    initFromFrames(frames);
}



Mat3::Mat3(const cv::Mat& im1, const cv::Mat& im2)
{
    cv::Mat tmp1 = im1.clone();
    cv::Mat tmp2 = im2.clone();
    if (tmp1.type() != CV_64FC1)
        tmp1.convertTo(tmp1, CV_64FC1);


    if (tmp2.type() != CV_64FC1)
        tmp2.convertTo(tmp2, CV_64FC1);

    frames.push_back(tmp1.clone());
    frames.push_back(tmp2.clone());
    initFromFrames(frames);
}

bool Mat3::areFramesLegal() const
{
    for (size_t i=0; i<frames.size(); ++i) {
        if (!frames[i].cols)
            return false;
...
Mat3::Mat3(const std::string &basepath, size_t T)
{
    for  (size_t t=0; t<T; ++t) {
        std::stringstream ss;
        ss << basepath << "_im" <<t << ".png";

        cv::Mat tmp = cv::imread(ss.str(), 0);
...
Mat3 Mat3::syntehsiseFromTthFrame(size_t t) const
{
    cv::Mat frame = frames[t];

    std::vector<cv::Mat> ims;
    for (size_t i=0; i<T(); ++i)
        ims.push_back(frame.clone());

    return Mat3(ims);
}

void Mat3::initFromFrames(const std::vector<cv::Mat>& ims)
{
    int dims[] = {(int)ims.size(), ims[0].rows, ims[0].cols};
    mat = std::shared_ptr<cv::Mat>(new cv::Mat(3, dims, CV_64FC1, cv::Scalar::all(0)));

    for (int t=0; t<T(); ++t)
        for (int y=0; y<rows(); ++y)
            for (int x=0; x<cols(); ++x)
                mat->at<double>(t,y,x) = ims[t].at<double>(y,x);

//    mat = cv::Mat(matToFill.clone());
}

std::vector<cv::Mat> Mat3::split() const
{
    ...
    for (int t=0; t<T(); ++t)
    {
        cv::Mat tmp(rows(), cols(), CV_64FC1, cv::Scalar::all(0));

        for (int y=0; y<rows(); ++y)
            for (int x=0; x<cols(); ++x)
                tmp.at<double>(y,x) = mat->at<double>(t,y,x);
...
std::vector<double> Image::columnToVector(const cv::Mat& col)
{
    std::vector<double> out;
    out.reserve(col.cols);
    for (size_t i=0; i<col.cols; i++)
        out.push_back(col.at<double>(0,i));

    return out;
}


double Image::matEnergy(const cv::Mat& mat)
{
    ...
    for (size_t i=0; i<mat.rows; ++i)
        for (size_t j=0; j<mat.cols; ++j)
            energy += mat.at<double>(i,j)*mat.at<double>(i,j);

    return energy;
}

cv::Mat Image::vectorToColumn(const std::vector<double>& vec)
{
    cv::Mat out(1,vec.size(),CV_64FC1, cv::Scalar::all(0));
    for (size_t i=0; i<vec.size(); i++)
        out.at<double>(0,i) = vec[i];
...
//function to read MAT from File
std::vector<int> Image::readFileToIntVector(const std::string& FileName) {
...
cv::Mat Image::doubleToUcharNoNorm(const cv::Mat& input)
{
...
//function to read MAT from File
void Image::writeVectorToFile(const std::vector<int>& ints, const std::string& fileName) {
...
//function to read MAT from File
void Image::writeValueToFile(double value, const std::string& fileName) {
...
std::vector<DetectionRect> cvip::eliminateCandidates(const std::vector<DetectionRect> &detections, int numOfSelected)
{
    ...
    cv::sortIdx(means, ordered, cv::SORT_EVERY_ROW + cv::SORT_ASCENDING);
...
{
    if (Im.channels() > 1)
        cv::cvtColor(Im, I, cv::COLOR_RGB2GRAY);
    else
        I = Im;
 ...
 */
cv::Mat Image::lbp(const cv::Mat& _in, const uint d)
{
    cv::Mat in = _in.clone();
    if (in.type() != CV_8U)
...
void Image::mct(const cv::Mat &I, cv::Mat &mI)
{
    // mI is 1px smaller from each side because the MCT cannot be computed on edges
    for (int i=1; i<I.rows-1; i++)
        for (int j=1; j<I.cols-1; j++)
        {
            float mean = (I.at<uchar>(i-1,j-1) + I.at<uchar>(i-1,j) + I.at<uchar>(i-1,j+1) +
                            I.at<uchar>(i,j-1) + I.at<uchar>(i,j) + I.at<uchar>(i,j+1) +
                            I.at<uchar>(i+1,j-1) + I.at<uchar>(i+1,j) + I.at<uchar>(i+1,j+1))/9.0f;
            ushort val = 0;

            val |= (ushort)(I.at<uchar>(i-1,j-1) > mean) << 8;
            val |= (ushort)(I.at<uchar>(i-1,j) > mean) << 7;
            val |= (ushort)(I.at<uchar>(i-1,j+1) > mean) << 6;
            val |= (ushort)(I.at<uchar>(i,j-1) > mean) << 5;
            val |= (ushort)(I.at<uchar>(i,j) > mean) << 4;
            val |= (ushort)(I.at<uchar>(i,j+1) > mean) << 3;
            val |= (ushort)(I.at<uchar>(i+1,j-1) > mean) << 2;
            val |= (ushort)(I.at<uchar>(i+1,j) > mean) << 1;
            val |= (ushort)(I.at<uchar>(i+1,j+1) > mean);

            mI.at<ushort>(i-1,j-1) = val;
        }
}


void Image::show(const cv::Mat &I, const std::string &winName)
{
#ifdef GUI
    cv::imshow(winName,cvip::Image::doubleToUchar(I));
    cv::waitKey(0);
#endif
}
...
//function to read MAT from File
double Image::readValueFromFile(const std::string& FileName) {

    double value;
    std::ifstream DataFile(FileName.c_str());
    if (DataFile.is_open()) {
        DataFile  >> std::setprecision(16) >> value;
        DataFile.close();
    }
    else {
        double erroneous;
...
//function to read MAT from File
cv::Mat Image::readFromFile(const std::string& FileName,  int cols) {

    cv::Mat Data = cv::Mat(0, cols, CV_64FC1);
    std::ifstream DataFile(FileName.c_str());
    if (DataFile.is_open()) {
        ...
        while (true)
        {
            ...
            cv::Mat tmp(1,cols,CV_64FC1);
            uint j;
            for (j = 0; j < Data.cols; j++) {
                if(DataFile.eof())
                    break;
                DataFile >> std::setprecision(16) >> tmp.at<double>(0,j);
            }

            if (j != Data.cols)
                break;

            Data.push_back(tmp.clone());
        }
...
void Image::zNormalize(cv::Mat& input, double mean, double std)
{
    ...
    for (int reci = 0; reci < input.rows; reci++) {
        for (int recj = 0; recj < input.cols; recj++) {
            input.at<double>(reci,recj) = (input.at<double>(reci,recj) - (mean+1e-10)) /  (std+1e-10);
        }
    }
}
...
void Image::printMat(const cv::Mat& mat)
{
    for (size_t i=0; i<mat.rows; ++i)
    {
        for (size_t j=0; j<mat.cols; ++j)
