...
/**
 * @brief LightGaborBank::GaborBank
 * @param _numPartsX
 * @param _numPartsY
 * @param _featureType
 ...
 */
LightGaborBank::LightGaborBank(size_t _numPartsX,
                     size_t _numPartsY,
...
std::string LightGaborBank::computeUniqueKey(const string &_featureType,
                                        ...
                                        const string &_orientsBitsetStr)
{
    ...
    ss << "Light-" << _timeWindow  << "-" << _numParts << "-" << _numParts << "-" <<  _scalesBitsetStr << "-" << _orientsBitsetStr << "-" << _featureType << "-" << _normalisationType;

    return ss.str();
}


std::vector<double> LightGaborBank::computeFeatures(const cvip::Mat3& mat) const
{
...
std::vector<cv::Mat> LightGaborBank::computeEnergies(tuple<Mat, Mat>& tup, const std::vector<double>& _scales,
...
/**
 * @brief LightGaborBank::computeNumFeatures - compute feature vector length from given feature combination
 * @param _featureType
 * @param _numParts
 * @param orients
 * @param scales
 * @return
 */
size_t LightGaborBank::computeNumFeatures(const std::string& _featureType,
                                     size_t _numParts,
...
double LightGaborBank::signalEnergy(const std::vector<double> &x) const
{
    double energy = 0;
    double scale = 0.;
    size_t i;
...
/**
 * @brief LightGaborBank::_computeFeatures -- compute features straight from image pair
 * @param tup - image pair
 * @return
 */
std::vector<double> LightGaborBank::_computeFeatures(const cvip::Mat3& mat3d) const
{
...
void LightGaborBank::extractSlice( const cv::Mat& image3d, const int z, cv::Mat& slice )
{
    // create the roi
    cv::Range ranges[3];
...
/**
             * @brief LightGaborBank::createFilter
             ...
             * @return
             */
std::tuple<cv::Mat, cv::Mat> LightGaborBank::createFilterTuple(int w, int h, double th, double v, double phi)
{
    double lam = 2*std::sqrt(1+v*v); // 2*std::sqrt(1+v*v)
    double sig = 0.4*lam*0.56; // lam*0.56
    double gam = 0.4; //% 0.75
    double mut = 0.55; // 1.75
    double tau = 2.75; // 2.75
...
/**
             ...
             * @return
             */
std::vector<cv::Mat> LightGaborBank::createFilterTuple(int w, int h, size_t T, double th, double v, double phi)
{
...
void LightGaborBank::createFilters()
{
    ...
    for (size_t i=0; i<orients.size(); ++i)
    {
        ...
        */
        reFilters.push_back(createFilterTuple(7, 7, orients[i], 1, 0));
        imFilters.push_back(createFilterTuple(7, 7, orients[i], 1, cvip::PI/2.));

        reFilters2.push_back(createFilterTuple(7, 7, timeWindow, orients[i], 1, 0));
        imFilters2.push_back(createFilterTuple(7, 7, timeWindow, orients[i], 1, cvip::PI/2.));
    }
    return;
}
